  ...
  for (i in 1:xDim){
    for (j in 1:yDim){
      ...
      if(beeMat[i,j]==3 & colMat[i,j]==5){
        if(runif(1, 1,100)<=sample(probDep, 1, replace=T)){
          colMat[i,j] <- 6
        }
      }
...
mx <- max(counter[-c(1:20),5]/(counter[-c(1:20),5]+counter[-c(1:20),3]))  
mu <- mean(counter[-c(1:20),5]/(counter[-c(1:20),5]+counter[-c(1:20),3]))  
mn <- min(counter[-c(1:20),5]/(counter[-c(1:20),5]+counter[-c(1:20),3])) 
...
probDep1 <- 22 #of depositing virus on flower
...
probVec <- rep(seq(0, 100, 5), 5)
StoreMat <- matrix(nrow=length(probVec), ncol=3)


for(i in 1:length(probVec)){
StoreMat[i,] <- ModelFunc(probAquireInfected = probVec[i], probDep = 22)
...
probVec1 <- rep(seq(0, 100, 5), 5)
StoreMat1 <- matrix(nrow=length(probVec1), ncol=3)


for(i in 1:length(probVec1)){
  StoreMat1[i,] <- ModelFunc(probAquireInfected = 30, probDep = probVec1[i])
